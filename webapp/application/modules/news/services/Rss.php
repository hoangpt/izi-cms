 ...
 * @version 	$Id: Rss.php 5048 2010-08-28 18:15:15Z hoangpt $
 */
...
class News_Services_Rss 
{
	...
	public static function feed($categoryId = null, $lang) 
	{
		...
		$prefix  = IZI_TEMP_DIR . DS . 'cache' . DS . $lang . '_news_rss_';
		
		$file 	 = (null == $categoryId)
					? $prefix . 'latest.xml' 
					: $prefix . 'category_' . $categoryId . '.xml';

		if (Izi_Cache_File::isFileNewerThan($file, time() - self::CACHE_TIME)) {
		...
		$newsConfig = Izi_Module_Config::getConfig('news');			
		...
		$generator = ($newsConfig->rss->channel_generator != '') 
						...
						: 'izicms v' . Izi_Version::getVersion(); 
