 ...
 * @version 	$Id: FileController.php 4761 2010-08-21 18:52:33Z hoangpt $
 * @since		2.0.0
 */
...
class Upload_FileController extends Zend_Controller_Action 
{
	...
	public function browseAction()
	{
		...
		$fullPath = IZI_ROOT_DIR . DS . $path;
		if (!file_exists($fullPath)) {
			$fullPath = IZI_ROOT_DIR . DS . 'upload';
			$path     = 'upload';
		}
	...
	public function uploadAction()
	{
		 ...
		 * Prepare folders
		 */
		$dir  = IZI_ROOT_DIR . DS . 'upload';
		$path = implode(DS, array($module, $userName, date('Y'), date('m')));
		Izi_Utility_File::createDirs($dir, $path);
		...
		if ($thumbnailSizes != 'none') {
			...
			foreach ($thumbnailSizes as $s) {
				 ...
				 */
				if ($watermark) {
					$service->setWatermarkFont(IZI_ROOT_DIR . '/data/upload/' . self::WATERMARK_FONT);
