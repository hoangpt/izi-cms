 ...
 * @version 	$Id: AuthController.php 4539 2010-08-12 09:56:40Z hoangpt $
 * @since		2.0.0
 */
...
class Core_AuthController extends Zend_Controller_Action 
{
	...
	public function init() 
	{
		Zend_Layout::getMvcInstance()
				->setLayoutPath(IZI_APP_DIR . DS . 'templates' . DS . 'admin' . DS . 'layouts')
				->setLayout('auth');
	}
	...
	public function forgotAction()
	{
		...
		if ($request->isPost()) {
			...
			if (count($users) == 0) {
				...
				$templateDao = Izi_Model_Dao_Factory::getInstance()->setModule('mail')->getTemplateDao();
		...
		if ($request->isPost()) {
			...
			switch ($result->getCode()) {
				...
				case Core_Services_Auth::SUCCESS:
					...
					Izi_Hook_Registry::getInstance()->executeAction('Core_Auth_Login_LoginSuccess', $user);
					$this->_redirect($this->view->serverUrl() . $this->view->url(array(), 'core_dashboard_index'));
					break;
	...
	public function logoutAction() 
	{
		...
		if ($auth->hasIdentity()) {
			...
			Izi_Hook_Registry::getInstance()->executeAction('Core_Auth_Login_LogoutSuccess', $user);
