 ...
 * @version 	$Id: PluginController.php 4535 2010-08-12 09:51:49Z hoangpt $
 * @since		2.0.0
 */
...
class Core_PluginController extends Zend_Controller_Action 
{
	...
	public function configAction() 
	{
		...
		if ($request->isPost()) {
			...
			Izi_Plugin_Config::saveParams($plugin, $params);
	...
	public function installAction() 
	{
		...
		if ($request->isPost()) {
			...
			$info = Izi_Plugin_Config::getPluginInfo($name);
			if ($info) {
	...
	{
		$plugins = array();
		$subDirs = Izi_Utility_File::getSubDir(IZI_APP_DIR . DS . 'plugins');
		foreach ($subDirs as $pluginName) {
			$info = Izi_Plugin_Config::getPluginInfo($pluginName);
			if (null == $info) {
				continue;
			}
			$plugin = new Core_Models_Plugin($info);
			$plugin->params = Izi_Plugin_Config::getParams($pluginName); 
			$plugins[] = $plugin;
		}
	...
	public function uploadAction() 
	{
		...
		if ($request->isPost()) {
			...
			if ($res === true) {
				...
				$info 	 = Izi_Plugin_Config::getPluginInfoFromXml($xml);
				if ($info) {
					$plugin = new Core_Models_Plugin($info);
					 ...
					 */
					$pluginDir = IZI_APP_DIR . DS . 'plugins' . DS . $plugin->name;
					Izi_Utility_File::copyRescursiveDir($tempDir . DS . $subDirs[0], $pluginDir);
				} else {
					/**
