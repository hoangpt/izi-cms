 ...
 * @version 	$Id: PageController.php 5376 2010-09-10 07:40:05Z hoangpt $
 * @since		2.0.0
 */
...
class Core_PageController extends Zend_Controller_Action 
{
	...
	public function addAction() 
	{
		...
		$pageDao = Izi_Model_Dao_Factory::getInstance()->setModule('core')->getPageDao();
	...
	public function layoutAction() 
	{
		 ...
		 */
		$jsonData = null;
		$jsonFile = IZI_APP_DIR . DS . 'templates' . DS . $template . DS . 'layouts' . DS . $pageName . '.json';
		$xmlFile  = IZI_APP_DIR . DS . 'templates' . DS . $template . DS . 'layouts' . DS . $pageName . '.xml';
	...
	public function savelayoutAction() 
	{
		...
		if ($request->isPost()) {
			 ...
			 */
			$jsonFile = IZI_APP_DIR . DS . 'templates' . DS . $template . DS . 'layouts' . DS . $page . '.json';
			$file 	  = fopen($jsonFile, 'w');
			fwrite($file, $jsonLayout);
			fclose($file);
			
			$xmlFile = IZI_APP_DIR . DS . 'templates' . DS . $template . DS . 'layouts' . DS . $page . '.xml';
			Izi_Layout::save($xmlFile, $layout);
	...
	public function widgetsAction() 
	{
		 ...
		 * Paginator
		 */
		$paginator = new Zend_Paginator(new Izi_Utility_PaginatorAdapter($widgets, $numWidgets));
