 ...
 * @version 	$Id: TranslationItems.php 4808 2010-08-24 03:27:26Z hoangpt $
 * @since		2.0.8
 */
...
class Core_View_Helper_TranslationItems
{
	...
	public function translationItems($item)
	{
		...
		if (!($item instanceof Izi_Model_Entity)) {
			throw new Exception('The item class (' . get_class($item) . ') has to extend from Izi_Model_Entity');
		}
		...
		$daoInstance = Izi_Model_Dao_Factory::getInstance()->setModule($array[0])->$daoClass();
