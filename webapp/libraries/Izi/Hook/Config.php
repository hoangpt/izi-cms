...
class Izi_Hook_Config 
{
	...
	public static function getHookInfo($hook, $module = null) 
	{
		...
		$file = (null == $module) 
				? IZI_APP_DIR . DS . 'hooks' . DS . $hook . DS . 'about.xml'
				: IZI_APP_DIR . DS . 'modules' . DS . $module . DS . 'hooks' . DS . $hook . DS . 'about.xml';
	...
	public static function getParams($hook, $module = null) 
	{
		$hook = strtolower($hook);
		$file = (null == $module)  
			? IZI_APP_DIR . DS . 'hooks' . DS . $hook . DS . 'config.xml'
			: IZI_APP_DIR . DS . 'modules' . DS . $module . DS . 'hooks' . DS . $hook . DS . 'config.xml';
		if (!file_exists($file)) {
			return null;
		}
		$xml = simplexml_load_file($file);
	...
	{
		$file = IZI_APP_DIR . DS . 'modules' . DS . $module . DS . 'config' . DS . 'hook.xml';
		if (!file_exists($file)) {
			return null;
		}
		$xml = simplexml_load_file($file);
