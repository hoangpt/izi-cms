 ...
 * @version 	$Id: UrlCache.php 5451 2010-09-16 08:23:32Z hoangpt $
 * @since		2.0.0
 */

class Izi_Controller_Plugin_UrlCache extends Zend_Controller_Plugin_Abstract 
{
	public function preDispatch(Zend_Controller_Request_Abstract $request) 
	{
		...
		if (Izi_Cache_File::isCached(Izi_Cache_File::CACHE_URL,
			$file, $lifetime)
		) {
			$request->setModuleName('core')
					->setControllerName('Cache')
					->setActionName('html')
					->setParam('__cacheType', Izi_Cache_File::CACHE_URL)
	...
	public function dispatchLoopShutdown() 
	{
		...
		if ($request->getParam('__isCacheURL') == true) {
			...
			Izi_Cache_File::cache(Izi_Cache_File::CACHE_URL, $key, $content);
		}
	}
}
