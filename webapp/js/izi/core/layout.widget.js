 ...
 * @version 	$Id: layout.widget.js 5352 2010-09-09 03:21:56Z hoangpt $
 * @since		2.0.0
 */

'Izi.Core.Layout.Widget'.namespace();
 ...
 * @param Izi.Core.Layout.Container container Widget container
 * @constructor
 */
Izi.Core.Layout.Widget = function(id, container) {
	/**
	 * Id of widget
	 */
	this._id = id;
	 ...
	 * Widget's class name
	 */
	this._class = 'Izi.Core.Layout.Widget';
 ...
 */
Izi.Core.Layout.Widget.CACHE_LIFETIME_PARAM = '___cacheLifetime';
Izi.Core.Layout.Widget.LOAD_AJAX_PARAM 	   = '___loadAjax';
Izi.Core.Layout.Widget.PREVIEW_MODE_PARAM   = '___widgetPreviewMode';

/**
 * Getters/Setters
 */
Izi.Core.Layout.Widget.prototype.getId = function() { return this._id; };
Izi.Core.Layout.Widget.prototype.setModule = function(module) { this._module = module; };
Izi.Core.Layout.Widget.prototype.setName = function(name) { this._name = name; };
Izi.Core.Layout.Widget.prototype.setTitle = function(title) { this._title = title; };
Izi.Core.Layout.Widget.prototype.getContainer = function() { return this._container; };
Izi.Core.Layout.Widget.prototype.setContainer = function(container) { this._container = container; };
 ...
 * @return void
 */
Izi.Core.Layout.Widget.prototype.render = function(params, showOnlyContent) {
	var div = $('<div/>');
	...
	$.ajaxq('core_layout', {
		...
		success: function(response) {
			...
			$('<div class="t_a_ui_layout_editor_widget_head"><h3>' + self._title + '</h3></div>'
				...
				+ 			Izi.Core.Layout.Lang.getLang('WIDGET_CACHE') + ':<br /><input type="text" style="width: 100px" name="' + Izi.Core.Layout.Widget.CACHE_LIFETIME_PARAM + '" class="t_widget_input" />'
				+ 			'<hr /><input type="checkbox" name="' + Izi.Core.Layout.Widget.LOAD_AJAX_PARAM + '" class="t_widget_input" /> ' + Izi.Core.Layout.Lang.getLang('WIDGET_LOAD_AJAX')
				+ 		'</div>'
				+ 		'<div class="t_a_ui_layout_editor_widget_preview" id ="' + $(div).attr('id') + '_preview" style="display: none"></div>'
				+ 	'</div>'
				+	'<div class="t_a_ui_layout_editor_widget_bottom"><a href="javascript: void(0);">' + Izi.Core.Layout.Lang.getLang('WIDGET_PREVIEW') + '</a></div>'
				).appendTo($(div));
			...
			$('<a href="javascript: void(0)" class="t_a_ui_layout_editor_widget_remove">CLOSE</a>').mousedown(function(e) {
            	...
            	if (confirm(Izi.Core.Layout.Lang.getLang('WIDGET_REMOVE_CONFIRM'))) {
			...
			$('<a href="javascript: void(0)" class="t_a_ui_layout_editor_widget_clone">CLONE</a>').mousedown(function(e) {
            	 ...
            	 */
            	var widget = new Izi.Core.Layout.Widget(widgetId, self._container);
 ...
 * @return void
 */
Izi.Core.Layout.Widget.prototype.preview = function(showOnlyContent) {
	this._mode = 'PREVIEW';	
	var params = {};
	 ...
	 */
	params[Izi.Core.Layout.Widget.PREVIEW_MODE_PARAM] = true;
 ...
 */
Izi.Core.Layout.Widget.prototype.save = function() {
	var out = {
		cls: this._class,
		module: this._module,
		name: this._name,
	...
	$('#' + self._id).find('.t_widget_input').each(function() {
		...
		if (name == Izi.Core.Layout.Widget.LOAD_AJAX_PARAM) {
 ...
 * @return void
 */
Izi.Core.Layout.Widget.prototype.toggleMode = function(mode, showOnlyContent) {
	switch (mode) {
		case 'PREVIEW':
			if (this._mode == 'PREVIEW') {
				...
				$('#' + this._id).find('div.t_a_ui_layout_editor_widget_bottom a').html(Izi.Core.Layout.Lang.getLang('WIDGET_BACK'));
			}
			break;
		case 'CONFIG':
			...
			$('#' + this._id).find('div.t_a_ui_layout_editor_widget_bottom a').html(Izi.Core.Layout.Lang.getLang('WIDGET_PREVIEW'));
