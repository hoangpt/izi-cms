 ...
 * @version 	$Id: layout.container.js 5082 2010-08-29 12:07:45Z hoangpt $
 * @since		2.0.0
 */

'Izi.Core.Layout.Container'.namespace();
 ...
 * @constructor
 */
Izi.Core.Layout.Container = function(id, parent) {
	/** 
	 * Id of DIV container
	 */
	this._id = id;
 ...
 */
Izi.Core.Layout.Container.TOTAL_COLUMNS = 12;

/**
 * Initial height for each created container 
 */
Izi.Core.Layout.Container.ROW_HEIGHT = 150;

/**
 * This static variable store the current dragged widget
 * TODO: Use jQuery data variable
 */
Izi.Core.Layout.Container.currentDraggedWidget = null;

/**
 * Getters/setters
 */
Izi.Core.Layout.Container.prototype.getId = function() { return this._id; };
Izi.Core.Layout.Container.prototype.setParent = function(parent) { this._parent = parent; };
Izi.Core.Layout.Container.prototype.getNumColumns = function() { return this._numColumns; };
Izi.Core.Layout.Container.prototype.setNumColumns = function(numColumns) { this._numColumns = numColumns; };
Izi.Core.Layout.Container.prototype.getPosition = function() { return this._position; };
Izi.Core.Layout.Container.prototype.setPosition = function(position) { this._position = position; };
Izi.Core.Layout.Container.prototype.setBgColor = function(color) { this._bgColor = color; };
Izi.Core.Layout.Container.prototype.getBaseUrl = function() { return this._baseUrl; };
Izi.Core.Layout.Container.prototype.setBaseUrl = function(url) { this._baseUrl = url; };
 ...
 */
Izi.Core.Layout.Container.prototype.isRoot = function() {
 ...
 * @return void 
 */
Izi.Core.Layout.Container.prototype._droppable = function() {
	var self = this;
	$('#' + self._id).droppable({
		greedy: true,
		...
		drop: function(event, ui) {
			...
			if ($(item).hasClass('t_column_draggable')) {
				...
				if ((self.isRoot() && numColumns == Izi.Core.Layout.Container.TOTAL_COLUMNS) 
					|| (!self.isRoot() && numColumns < Izi.Core.Layout.Container.TOTAL_COLUMNS)) {
					
					if (numColumns != Izi.Core.Layout.Container.TOTAL_COLUMNS && self.getChildTotalColumns() > Izi.Core.Layout.Container.TOTAL_COLUMNS) {
						/**
			 ...
			 */
				...
				self._tabUuid++;
				var tab = new Izi.Core.Layout.Tabs(tabId, self);
				tab.render();
			 ...
			 */
				...
				var arr = $(item).attr('id').split('_');
				var widget = new Izi.Core.Layout.Widget(widgetId, self);
			 ...
			 */
				...
				var widgetId = self.generateWidgetId();
				var output = new Izi.Core.Layout.DefaultOutput(widgetId, self);
 ...
 * @return void
 */
Izi.Core.Layout.Container.prototype.sortable = function() {
	var self = this;
	$('.t_a_ui_layout_editor_container').sortable({
        ...
        start: function(e, ui) {
			...
			Izi.Core.Layout.Container.currentDraggedWidget = self.getWidget(widgetId);
        ...
        receive: function(e, ui) {
        	 ...
        	 */
        	if (Izi.Core.Layout.Container.currentDraggedWidget != null) {
        		Izi.Core.Layout.Container.currentDraggedWidget.setContainer(self);
	        	self.addWidget(Izi.Core.Layout.Container.currentDraggedWidget);
        	}
        },
        stop: function(e, ui) {
        	/**
        	 ...
        	 */
        	if (Izi.Core.Layout.Container.currentDraggedWidget != null
        			&& Izi.Core.Layout.Container.currentDraggedWidget.getContainer().getId() != self._id) {
        		self.removeWidget(Izi.Core.Layout.Container.currentDraggedWidget);
        	}
	 ...
	 */
	if (this._numColumns == Izi.Core.Layout.Container.TOTAL_COLUMNS) {
 ...
 * @return void
 */
Izi.Core.Layout.Container.prototype.resizable = function() {
	if (this._numColumns == Izi.Core.Layout.Container.TOTAL_COLUMNS) {
		return;
	}
	var self = this;
	$('#' + this._id).resizable({
		...
		maxWidth: 940,
		handles: 'e',
		//maxHeight: Izi.Core.Layout.Container.ROW_HEIGHT,
		ghost: false,
		grid: [80, 0],
		containment: 'parent',
		start: function(e, ui) {
 ...
 * @return void
 */
Izi.Core.Layout.Container.prototype.updatePositionType = function() {
	var total = 0;
	...
	for (var i = 0; i < child.length; i++) {
		...
		$('#' + $(child[i]).attr('id')).find('.t_a_ui_layout_editor_container_head:first h3').html(sprintf(Izi.Core.Layout.Lang.getLang('CONTAINER_COLS'), childColumns));
 ...
 * @return int
 */
Izi.Core.Layout.Container.prototype.getChildTotalColumns = function() {
 ...
 * @return Izi.Core.Layout.Container The child container has just been added
 */
Izi.Core.Layout.Container.prototype.append = function(numColumns) {
	/**
	...
	if (numColumns != Izi.Core.Layout.Container.TOTAL_COLUMNS
			&& totalColumns > Izi.Core.Layout.Container.TOTAL_COLUMNS) {
		return null;
	}
...
//	});
	 ...
	 */
	$('#' + this._id).append(div);
	var childContainer = new Izi.Core.Layout.Container(id, this);
	 ...
	 */
	var self = this;
	$('<div class="t_a_ui_layout_editor_container_head"><h3>' + sprintf(Izi.Core.Layout.Lang.getLang('CONTAINER_COLS'), numColumns) + '</h3></div>').css('cursor', 'move').appendTo($(div));
	$('<a href="javascript: void(0)" class="t_a_ui_layout_editor_container_remove">REMOVE</a>').mousedown(function(e) {
        e.stopPropagation();
    }).click(function() {
    	if (confirm(Izi.Core.Layout.Lang.getLang('CONTAINER_REMOVE_CONFIRM'))) {
    		$(div).remove();
	...
	var minHeight = Izi.Core.Layout.Container.ROW_HEIGHT + 'px';
	...
	if (/*totalColumns == numColumns || */ numColumns == this._numColumns || this._childCount == 0) {
		...
		if (numColumns == Izi.Core.Layout.Container.TOTAL_COLUMNS) {
			minHeight += Izi.Core.Layout.Container.ROW_HEIGHT + 10;
 ...
 * Add child container
 * 
 * @param Izi.Core.Layout.Container container
 * @return void
 */
Izi.Core.Layout.Container.prototype.addChildContainer = function(container) {
 ...
 * 
 * @param string id
 * @return Izi.Core.Layout.Container
 */
Izi.Core.Layout.Container.prototype.getChildContainer = function(id) {
 ...
 * @return void
 */
Izi.Core.Layout.Container.prototype.removeChildContainer = function(container) {
 ...
 * Add widget
 * 
 * @param Izi.Core.Layout.Widget widget
 * @return void
 */
Izi.Core.Layout.Container.prototype.addWidget = function(widget) {
 ...
 * @return Izi.Core.Layout.Widget
 */
Izi.Core.Layout.Container.prototype.getWidget = function(id) {
 ...
 * 
 * @param Izi.Core.Layout.Widget widget The widget will be removed
 * @return void
 */
Izi.Core.Layout.Container.prototype.removeWidget = function(widget) {
 ...
 * @return void
 */
Izi.Core.Layout.Container.prototype.incHeight = function(added) {
 ...
 */
Izi.Core.Layout.Container.prototype.generateWidgetId = function() {
	this._widgetUuid++;
 ...
 * @return string
 */
Izi.Core.Layout.Container.prototype.generateTabId = function() {
 ...
 */
Izi.Core.Layout.Container.prototype.save = function() {
	var out = {
 ...
 * @return void
 */
Izi.Core.Layout.Container.prototype.load = function(data, showOnlyContent) {
	...
	for (i in data.widgets) {
		...
		switch (data.widgets[i].cls) {
			case 'Izi.Core.Layout.DefaultOutput':
				widget = new Izi.Core.Layout.DefaultOutput(widgetId, this);	
				break;
			case 'Izi.Core.Layout.Widget':
			default:
				widget = new Izi.Core.Layout.Widget(widgetId, this);
				break;
		}
 ...
 * @return void
 */
Izi.Core.Layout.Container.prototype.toggleMode = function(mode, showOnlyContent) {
	switch (mode) {
		case 'PREVIEW':
			if (showOnlyContent) {
				$('#' + this._id).children('.t_a_ui_layout_editor_container_head').hide();
