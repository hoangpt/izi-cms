...
'Izi.Ad.Loader'.namespace();

Izi.Ad.Loader = function(zoneId, containerId, route, url) {
	this._zoneId 	  = zoneId;
...
Izi.Ad.Loader.prototype = {
	...
	load: function() {
		 ...
		 */
		var zone = new Izi.Ad.Zone(gZone.id, gZone.name, gZone.width, gZone.height);
		...
		for (var i = 0; i < G_AD_BANNERS.length; i++) {
			...
			var banner = new Izi.Ad.Banner(gBanner.id, new Izi.Ad.Zone(gBanner.zone.id), gBanner.options, gBanner.baseUrl);
			
			pageUrl = banner.getOption('pageUrl');
			pageUrl = this._normalizeUrl(pageUrl);
			var regex = new RegExp(pageUrl, "g");
			
			var match = (url == '' && pageUrl == '') 
