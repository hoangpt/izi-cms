 ...
 * @version 	$Id: treetable.js 5082 2010-08-29 12:07:45Z hoangpt $
 * @since		2.0.7
 */

'Izi.TreeTable'.namespace();

/**
 * @constructor
 */
Izi.TreeTable = function(id, rowPrefix) {
	/**
...
Izi.TreeTable.CHILD_PADDING = 40;

Izi.TreeTable.prototype = {
	/**
	 * Initialize the rows
	 * 
	 * @return void
		...
		$(row).droppable({
			...
			drop: function(event, ui) {
				...
				for (var i = movingRows.length - 1; i >= 0; i--) {
					...
					$(tr).find('td:first')
						.css('padding-left', targetPaddingLeft + delta + Izi.TreeTable.CHILD_PADDING + 'px');
	...
	getTreeData: function() {
		...
		for (var i = 0; i < numRows; i++) {
			...
			depth = parseInt($(rows[i]).find('td:first').css('padding-left')) / Izi.TreeTable.CHILD_PADDING;
	...
	_decreaseDepth: function(row, step) {
		...
		$(td).css('padding-left', paddingLeft + step * Izi.TreeTable.CHILD_PADDING + 'px');
